...
export default class PartitionQueue {
    private _onQueueDrain: Function;
    private _q;
    private _lastProcessed: number;
    private _lastPushed: number;
    private _totalPushed: number;
    private _totalProcessedMessages: number;
    private _totalMessageProcessFails: number;
    private _lastOffset: number;
    private _drainCheckIntv;

    constructor(public partition, private _drainEvent, private _drainer, private asyncLimit = 1, queueDrain = null) {

        if (typeof _drainEvent !== "function") {
