...
class Kafka extends EventEmitter {
    ...
    becomeConsumer(topics = ["t"], groupId = "kafka-node-group", id = "my-consumer-id", _options = {}, dontListenForSIGINT = false){
    ...
    _attachConsumerListeners(dontListenForSIGINT = false){
        ...
        this.client.on("connect", () => {
            this._getLogger().info("consumer is connected.");
            this.emit("ready");
        });
        ...
        this.consumer.on("message", message => {
            ...
            try {
                ...
                return this.emit("error", "failed to json parse message value: " + message.id);
            }

            if(!message.value){
                return this.emit("error", "message value is empty: " + message.id);
            }

            this._updateConsumerOffsetPosition(message.topic, message.partition, message.offset);
    ...
    _updateConsumerOffsetPosition(topic, partition, offset){

        if(!this.internalConsumerOffset[topic]){
            this.internalConsumerOffset[topic] = {};
        }

        if(!this.internalConsumerOffset[topic][partition]){
            this.internalConsumerOffset[topic][partition] = 0;
        }

        if(offset > this.internalConsumerOffset[topic][partition]){
            this.internalConsumerOffset[topic][partition] = offset;
        }
    }
    ...
    _closeConsumer(commit = false) {
        return new Promise((resolve, reject) => {
            ...
            this._getLogger().info("trying to commit kafka consumer at " + JSON.stringify(this.internalConsumerOffset));
