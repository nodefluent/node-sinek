...
class JSConsumer extends EventEmitter {
   ...
   * @returns {boolean}
   */
  _singleConsumeRecursive(batchSize = 1, syncEvent) {
  ...
  async _consumeHandler(syncEvent, {
    ...
    if (!manualBatching) {
      throw new Error("The consumer only allow manual batching for now");
    }

    this.config.logger.info("Batching manually..");
    this._singleConsumeRecursive(batchSize, syncEvent);
  }
