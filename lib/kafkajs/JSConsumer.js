...
class JSConsumer extends EventEmitter {
  ...
  _singleConsumeRecursive(batchSize = 1) {
    ...
    if (this.config && this.config.logger && this.config.logger.debug) {
      this.config.logger.debug(`Consuming recursively.. ${batchSize}`);
    }

    this.consumer.run({
      eachBatchAutoResolve: false,
      eachBatch: async ({ batch, uncommittedOffsets, heartbeat, isRunning, isStale }) => {
        ...
        if (!isRunning() || isStale() || !messages.length) {
          ...
          if (this.config && this.config.logger && this.config.logger.debug) {
            this.config.logger.debug(`Consumed recursively for empty batch, backing off ${graceTime} ms after ${this._emptyFetches} attempts.`);
          }
          ...
          this._uncommitedOffsets = uncommittedOffsets();
          super.emit("batch", batch.messages);
        }
        await heartbeat();
      }
    });
  ...
  consume(syncEvent = null, asString = true, asJSON = false, options = {}) {
    ...
    manualBatching = typeof manualBatching === "undefined" ? false : manualBatching; //default is false
          ...
          super.on("batch", (messages) => {
            ...
            if (sortedManualBatch) {
              mappedMessages.forEach((mappedMessage) => {
                ...
                if (typeof sortedBatch[mappedMessage.topic][mappedMessage.partition] === "undefined") {
                  sortedBatch[mappedMessage.topic][mappedMessage.partition] = [];
                }

                sortedBatch[mappedMessage.topic][mappedMessage.partition].push(mappedMessage);
              });
            }

            //execute sync event and wrap callback (in this mode the sync event recieves all messages as batch)
            syncEvent(!sortedManualBatch ? mappedMessages : sortedBatch, async (__error) => {
              ...
              this._consumedSinceCommit += mappedMessages.length;
              this._totalBatches++;
              this._batchCount++;
              ...
              if (commitSync) {
                try {
                  await this.consumer.commitOffsets(this._uncommitedOffsets);
                } catch (error) {
                  super.emit("error", error);
                }
                this._singleConsumeRecursive(batchSize); //recall instant
              } else {
                this.consumer.commitOffsets(this._uncommitedOffsets);
                setTimeout(() => {
   ...
   */
  async commit(async = false) {

    if (!this.consumer || !this.topics || !this.topics.length) {
      return false;
    }

    let uncommittedOffsets;

    try {

      uncommittedOffsets = [].concat.apply([],
        await Promise.all(
    ...
    if (async) {
      this.consumer.commitOffsets(uncommittedOffsets);
      return true;
    }

    try {
      await this.consumer.commitOffsets(uncommittedOffsets);
      return true;
    } catch (error) {
      super.emit("error", error);
      return false;
   ...
   * @returns {boolean}
   */
  async commitMessage(async = false, message) {
    ...
    if (async) {
      this.consumer.commitOffsets([message]);
      return true;
    }
  ...
  commitOffsetHard(topic, partition, offset, async = false) {
    ...
    const fakeMessage = {
      topic,
      partition,
      offset: offset - 1 // node-rd-kafka commitMessage => offset + 1
    };
  ...
  async commitLocalOffsetsForTopic(topic) {
    ...
    try {
      uncommittedOffsets = await (this._adminClient.fetchOffsets({
          groupId: this._groupId,
          topic,
        }))
       .map(offsetInfo => {
    ...
    return this.consumer.commitOffsets(uncommittedOffsets);
  }
