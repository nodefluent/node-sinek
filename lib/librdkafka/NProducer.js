...
const {ProducerAnalytics} = require("./Analytics.js");
...
class NProducer extends EventEmitter {
  ...
  getTopicMetadata(topic, timeout = 2500){
    return new Promise((resolve, reject) => {
      this.producer.getMetadata({topic, timeout}, (error, raw) => {
  ...
  getAnalytics(){
    ...
    if(!this._analytics){
      throw new Error("You have not enabled analytics on this consumer instance.");
    }
