...
class NProducer extends EventEmitter {
  ...
  connect() {
    return new Promise((resolve, reject) => {
      ...
      this.producer = new BlizzKafka.Producer(config);
  ...
  send(topic, message) {
    ...
    const topic = this.producer.Topic(topic, topicConfig); //TODO store on connect
    const key = uuid.v4();
    this.producer.produce(topic, partition, new Buffer(message), key);
