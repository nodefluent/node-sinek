...
class NConsumer extends EventEmitter {
  ...
  enableAnalytics(options = {}) {
    ...
    lagFetchInterval = lagFetchInterval || 1000 * 60 * 5; // 5 minutes
      ...
      if (!syncEvent) {
        ...
        if (manualBatching) {
          ...
          super.on("batch", (messages) => {
            ...
            syncEvent(!sortedManualBatch ? mappedMessages : sortedBatch, (__error) => {
  ...
  adjustSubscription(topics = []) {
    ...
    if (!topics.length) {
      this.config.logger.info("Unsubscribing from all topics.");
  ...
  async commitLocalOffsetsForTopic(topic) {
    ...
    return this.consumer.commit(currentLocalOffsets);
  }
