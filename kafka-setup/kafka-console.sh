...
case "$COMMAND" in
  "consume")
    ...
    create_client_properties
    KAFKA_LOG4J_OPTS="-DBASEDIR=$BASEDIR -Djava.security.auth.login.config=${CLIENT_JAAS}" kafka-console-consumer --from-beginning --bootstrap-server=${KAFKA_HOST} --topic=${TOPIC} --consumer.config=${CLIENT_PROPERTIES}
  ;;
  "produce")
    echo "Produce on ${TOPIC} topic."
    create_client_properties
    KAFKA_LOG4J_OPTS="-DBASEDIR=$BASEDIR -Djava.security.auth.login.config=${CLIENT_JAAS}" kafka-console-producer --broker-list=${KAFKA_HOST} --topic=${TOPIC} --producer.config=${CLIENT_PROPERTIES}
  ;;
  "create-topic")
    echo "Create topic:"
